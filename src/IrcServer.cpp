...
/*   Updated: 2024/06/12 12:27:48 by lucperei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	IRCServer::handleClientMessage(int client_fd) {
	...
	nbytes = read(client_fd, buffer, sizeof(buffer) - 1); // le os dados do client
	...
	if (message.substr(0,5) == "/file") {
		...
		_bot.respond_to_message(client_fd, message); // chamada para o bot para responder
	}
